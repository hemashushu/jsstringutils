// import GraphemeBreaker from '../src/GraphemeBreaker.mjs' // MOD::REM
// import chai from 'chai' // MOD::REM
// import fs from 'fs' // MOD::REM
// import punycode from 'punycode' // MOD::REM

const GraphemeBreaker = require('../src/GraphemeBreaker'); // MOD::ADD
const chai = require('chai'); // MOD::ADD
const fsPromise = require('fs/promises'); // MOD::ADD
const punycode = require('punycode/'); // Userland module // MOD::ADD
...
describe('GraphemeBreaker', function() {
  ...
  it('should pass all tests in GraphemeBreakTest.txt', async function() {
    var codePoints, cols, comment, data, expected, j, len, line, lines, results, str;
    // data = fs.readFileSync(__dirname + '/GraphemeBreakTest-13.0.0.txt', 'utf8'); // MOD::REM
    data = await fsPromise.readFile(__dirname + '/GraphemeBreakTest-13.0.0.txt', 'utf8'); // MOD::ADD
    lines = data.split('\n');
    results = [];
  ...
  it('should pass all tests in GraphemeBreakTest.txt in reverse', async function() {
  ...
  it('should pass all tests in emoji-test.txt', async function() {
    var codePoints, cols, comment, data, j, len, line, lines, results, str;
    // data = fs.readFileSync(__dirname + '/emoji-test.txt', 'utf8'); // MOD:: REM
    data = await fsPromise.readFile(__dirname + '/emoji-test.txt', 'utf8'); // MOD:: ADD
    lines = data.split('\n');
    results = [];
    for (j = 0, len = lines.length; j < len; j++) {
      line = lines[j];
  ...
  return it('should pass all tests in emoji-test.txt in reverse', async function() {
    var codePoints, cols, comment, data, j, len, line, lines, results, str;
    // data = fs.readFileSync(__dirname + '/emoji-test.txt', 'utf8'); // MOD::REM
    data = await fsPromise.readFile(__dirname + '/emoji-test.txt', 'utf8'); // MOD::ADD
    lines = data.split('\n');
